"""DensityChunker — LegalBench 1024-d. Tunes sigma on FULL dataset via Optuna, saves best."""
...
CACHE_PATH = PROJECT_ROOT / "results" / "segmented_docs_cache_1024.pkl"
OUT_PATH = PROJECT_ROOT / "results" / "main" / "density_legalbench_1024d.json"
DIM = 1024
N_TRIALS = 10
CHUNK_BATCH = 64
...
def objective(trial) -> float:
    ...
    indexer = ChromaIndexer(collection_name=f"opt_lb1024_{trial.number}")
...
def main():
    ...
    fixed = {}
    for key, val in _doc_data.items():
        parts = key.split("/", 1)
        if len(parts) == 2 and parts[0] == parts[1].split("/")[0]:
            ...
            fixed[key] = val
    _doc_data = fixed
    print(f"  {len(_doc_data)} docs (keys fixed)")
    ...
    indexer = ChromaIndexer(collection_name=f"final_lb1024")
    retriever = ChromaRetriever(indexer=indexer, embedder=_embedder, k=10)
    chunk_embs = _embedder.encode([ch.text for ch in flat_chunks], batch_size=CHUNK_BATCH)
    metadatas = [{"doc_id": ch.metadata["doc_id"], "start_char": ch.start_char, "end_char": ch.end_char} for ch in flat_chunks]
    indexer.add_chunks(chunk_ids=[ch.chunk_id for ch in flat_chunks], embeddings=chunk_embs, metadatas=metadatas)
