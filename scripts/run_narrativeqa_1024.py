"""NarrativeQA 1024-d baseline eval. Optimized: large batch sizes, parallel spaCy."""
...
def run_method(chunker, doc_data, samples, embedder):
    ...
    indexer = ChromaIndexer(collection_name=f"nqa1024_{chunker.name}")
...
def main():
    ...
    if CACHE_PATH.exists():
        ...
        print("Embedding sentences at 1024-d...")
        t0 = time.time()
    ...
    print(f"\n{'='*60}")
    print("FINAL RESULTS — NarrativeQA (1024-d baselines)")
    print(f"{'='*60}")
    ...
    out_path = PROJECT_ROOT / "results" / "main" / "narrativeqa_baselines_1024d.json"
